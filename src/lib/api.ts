...
export function listFavoriteTeams() { return request<FavoriteTeam[]>('/users/me/favorite-teams'); }
export function replaceFavoriteTeams(teams: FavoriteTeam[]) { return request<FavoriteTeam[]>('/users/me/favorite-teams', { method: 'PUT', body: { teams: teams.map(({ sport, teamName, nickname }) => ({ sport, teamName, nickname: nickname?.trim() || null })) } }); }
export function listFavoritePlaces() { return request<FavoritePlace[]>('/users/me/favorite-places'); }
export function addFavoritePlace(body: Omit<FavoritePlace, 'id' | 'createdAt'>) { return request<FavoritePlace>('/users/me/favorite-places', { method: 'POST', body }); }
export function deleteFavoritePlace(id: number) { return request<void>(`/users/me/favorite-places/${id}`, { method: 'DELETE' }); }
